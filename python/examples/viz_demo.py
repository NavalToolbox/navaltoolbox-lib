...
from navaltoolbox import (  # noqa: E402
    Vessel,
    Hull,
    Tank,
    Silhouette,
...
from navaltoolbox.visualization import (  # noqa: E402
    plot_hydrostatic_condition,
    plot_vessel_3d,
)
...
def main():
    ...
    # Assuming standard repo structure:
    # python/examples/viz_demo.py -> rust/tests/data/dtmb5415.stl
    ...
    if not stl_path.exists():
        ...
        if not stl_path.exists():
            print(
                "Could not locate dtmb5415.stl. Check working directory."
            )
            return
    ...
    vent = DownfloodingOpening.from_point(
        "Vent Stbd", (40.0, 6.0, deck_z + 1.0), OpeningType.vent()
    )
    ...
    fig = plot_vessel_3d(
        vessel, title="DTMB 5415 - Visualization Demo", opacity_hull=0.5
    )
    ...
    fig_hydro = plot_hydrostatic_condition(
        ...
        heel=15.0,
        trim=1.0,
        title="DTMB 5415 - Hydrostatic (T=6.15m, Heel=15°, Trim=1°, COG)",
        opacity_hull=0.5,
    ...
    print(
        f"Success! Hydrostatic saved to: {os.path.abspath(output_hydro)}"
    )

    # Open files automatically based on platform
    try:
        if sys.platform == "darwin":  # macOS
            ...
            os.startfile(output_hydro)
        elif sys.platform.startswith("linux"):  # Linux
            os.system(f"xdg-open {output_file}")
            os.system(f"xdg-open {output_hydro}")
