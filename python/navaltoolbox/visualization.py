...
def plot_vessel_3d(
    ...
    if show_tanks:
        for tank in vessel.get_tanks():
            ...
            if hasattr(tank, "get_fluid_vertices") and hasattr(tank, "get_fluid_faces"):
    ...
    if show_silhouettes:
        for sil in vessel.get_silhouettes():
            points = sil.get_points() # List[(x, y, z)] but y is usually 0 for silhouettes
            if not points:
                continue
    ...
    # Default to showing them if not explicitly disabled (could add a generic argument later)
    ...
    scene_args: dict[str, Any] = dict(
        aspectmode='cube'
    )
    
    if show_axes:
        scene_args.update(dict(
...
def plot_hydrostatic_condition(
    ...
    show_hulls: bool = True,
    show_tanks: bool = True,
    show_silhouettes: bool = True, # Silhouettes might look weird in 3D if they are 2D planes
    show_openings: bool = True,
    opacity_hull: float = 0.5,
    opacity_tank: float = 0.3,
    ...
    The waterplane is fixed at Z=0. The vessel is transformed (translated and rotated)
    to match the draft, trim, and heel.
    ...
    # Combined Rotation R = Ry @ Rx (apply heel first, then trim?? Or trim then heel?)
    # Naval architecture usually defines trim in the upright condition, or does it?
    # Usually Euler Y-X (Pitch-Roll) or X-Y. 
    # Let's assume standard sequence Yaw-Pitch-Roll (Z-Y-X). Yaw=0. So Pitch then Roll.
    # R = Rx @ Ry
    # R_mat = Rx @ Ry
    # If points are Row vectors (N, 3): v' = v @ R.T
    # Here we constructed rotation matrices for column vectors v' = R v.
    # So for row vectors: v' = v @ R.T
    # Equivalently v' = v @ (Rx @ Ry).T = v @ Ry.T @ Rx.T
    
    # Let's define R for v' = R @ v (Standard math)
    # Ry rotates around Y.
    # Combined: R = Rx @ Ry (Roll applied AFTER Pitch? Or Pitch after Roll?)
    # Valid question. Standard Tait-Bryan Z-Y-X applies transformation Rz then Ry then Rx (intrinsic) or Rx then Ry then Rz (extrinsic).
    # Let's assume Pitch then Roll.
    R_total = Rx @ Ry 
    ...
    def transform_points(points):
        ...
        # Apply translation
        # Z translation: -draft.
        # Ideally, we should pivot around the flotation center, but here we just translate vertically.
    ...
    wx = np.linspace(-plane_size/2, plane_size/2, 2) + (bounds[0]+bounds[1])/2
    ...
    outline_x = [cx - half_size, cx + half_size, cx + half_size, cx - half_size, cx - half_size]
    outline_y = [cy - half_size, cy - half_size, cy + half_size, cy + half_size, cy - half_size]
    outline_z = [0, 0, 0, 0, 0]
    ...
    if show_tanks:
        for tank in vessel.get_tanks():
            ...
            if hasattr(tank, "get_fluid_vertices"):
                # Pass vessel heel/trim to get the fluid mesh that is 'horizontal' in world
                # but 'tilted' in ship frame.
...
def _add_hull_trace(fig: go.Figure, hull: Hull, name: str, opacity: float):
    ...
    # Limitation: PyHull does not expose raw vertices/faces in Python wrapper yet.
    # It only exposes num_vertices, etc.
    # To plot mesh, we need vertices and faces indices.
    # I ADDED getters to PyVessel but I did NOT add accessors for mesh data to PyHull!
    # This is a critical missing piece for visualization.
    # I need to add `get_vertices()` and `get_faces()` (indices) to `PyHull`.
    # For now, I will assume they are available or I will add them in next steps.
    # Implementation Plan Step 2 assumed `hull.get_vertices()` and `hull.get_faces()`.
    
    # Placeholder implementation assuming methods exist:
    if hasattr(hull, "get_vertices") and hasattr(hull, "get_faces"):
        verts = hull.get_vertices()
        faces = hull.get_faces()
