...
fn test_dtmb5415_simman_validation() {
    ...
    let state = calc.calculate_at_draft(draft, 0.0, 0.0, Some(vcg))
        .expect("Failed to calculate hydrostatics");
        
    println!("Hydrostatic State at T={}: {:#?}", draft, state);
    
    // 1. Beam at Waterline (Bwl) - Ref: 19.06 m
    assert!((state.bwl - 19.06).abs() < 0.01, "Bwl mismatch: got {}, expected ~19.06", state.bwl);
    
    // 2. Length at Waterline (Lwl) - Ref: 142.18 m
    assert!((state.lwl - 142.18).abs() < 0.1, "Lwl mismatch: got {}, expected ~142.18", state.lwl);
    ...
    assert!((state.volume - 8424.0).abs() < 80.0, "Volume mismatch: got {}, expected ~8424", state.volume);
    
    // 4. Block Coefficient (CB) - Ref: 0.506
    assert!((state.cb - 0.506).abs() < 0.02, "CB mismatch: got {}, expected ~0.506", state.cb);
    ...
    assert!((state.cm - 0.816).abs() < 0.05, "Cm mismatch: got {}, expected ~0.816", state.cm);
    
    // 6. Wetted Surface Area (S) - Ref: 2972.6 m2
    assert!((state.wetted_surface_area - 2972.6).abs() < 20.0, "S mismatch: got {}, expected ~2973", state.wetted_surface_area);
    ...
    if let Some(gmt) = state.gmt {
        assert!((gmt - 1.95).abs() < 0.02, "GMt mismatch: got {}, expected ~1.95", gmt);
    } else {
        panic!("GMt was None");
    }
}
