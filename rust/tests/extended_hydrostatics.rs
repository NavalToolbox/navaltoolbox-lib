...
fn test_box_hydrostatics() {
    ...
    let state = calc.calculate_at_draft(draft, 0.0, 0.0, None)
    ...
    // Total = 300
    assert!((state.wetted_surface_area - 300.0).abs() < 1.0, 
        "Wetted Surface mismatch: got {}, expected 300.0", state.wetted_surface_area);

    // 2. Midship Area
    // 10 width * 5 draft = 50
    assert!((state.midship_area - 50.0).abs() < 0.5,
        "Midship Area mismatch: got {}, expected 50.0", state.midship_area);
    ...
    assert!((state.cb - 1.0).abs() < 0.01, "Cb mismatch: got {}, expected 1.0", state.cb);
    assert!((state.cm - 1.0).abs() < 0.01, "Cm mismatch: got {}, expected 1.0", state.cm);
    assert!((state.cp - 1.0).abs() < 0.01, "Cp mismatch: got {}, expected 1.0", state.cp);
    ...
    assert!((c33 - expected_c33).abs() < 100.0, 
        "C33 stiffness mismatch: got {}, expected {}", c33, expected_c33);

    // 5. LOS
    // Should be 10.0
    assert!((state.los - 10.0).abs() < 0.1, 
        "LOS mismatch: got {}, expected 10.0", state.los);
}
...
fn test_box_displacement_calculation() {
    ...
    let state = calc.calculate_at_displacement(target_disp, None, None, None)
        .expect("Failed to calculate at displacement");

    // Check draft
    assert!((state.draft - 5.0).abs() < 0.01, 
        "Draft mismatch for displacement: got {}, expected 5.0", state.draft);

    // Check properties propagate
    assert!((state.wetted_surface_area - 300.0).abs() < 1.0,
        "WSA mismatch in displacement calc: got {}", state.wetted_surface_area);
    assert!((state.cb - 1.0).abs() < 0.01,
        "Cb mismatch in displacement calc: got {}", state.cb);
}
