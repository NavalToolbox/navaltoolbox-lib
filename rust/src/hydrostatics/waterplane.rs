...
pub fn calculate_waterplane_properties(
    mesh: &TriMesh,
    draft: f64,
) -> Option<WaterplaneProperties> {
    ...
    for contour in &contours {
        ...
        for tri in triangles {
            ...
            let i_xx_tri = area * (p0.y * p0.y + p1.y * p1.y + p2.y * p2.y + 
                                     p0.y * p1.y + p1.y * p2.y + p2.y * p0.y) / 6.0;
            let i_yy_tri = area * (p0.x * p0.x + p1.x * p1.x + p2.x * p2.x + 
                                     p0.x * p1.x + p1.x * p2.x + p2.x * p0.x) / 6.0;
...
fn extract_waterline_contours(
    ...
    let mut points: Vec<Point3<f64>> = crossing_segments.iter()
        .map(|(p, _)| *p)
        .collect();
...
mod tests {
    ...
    fn test_box_waterplane_area() {
        ...
        assert!((wp.area - expected_area).abs() < 1.0, 
                "Area: got {:.2}, expected {:.2}", wp.area, expected_area);
    }
    ...
    fn test_box_waterplane_centroid() {
        ...
        assert!((wp.centroid[0] - 5.0).abs() < 0.1,
                "LCF: got {:.2}, expected 5.0", wp.centroid[0]);
        assert!(wp.centroid[1].abs() < 0.1,
                "TCF: got {:.2}, expected 0.0", wp.centroid[1]);
    }
    ...
    fn test_box_metacentric_heights() {
        ...
        assert!((state.bmt - expected_bmt).abs() < 0.1,
                "BMt: got {:.3}, expected {:.3}", state.bmt, expected_bmt);
        
        // BM_l = L² / (12×T) = 100 / 60 = 1.667 m (same for square)
        assert!((state.bml - expected_bmt).abs() < 0.1,
                "BMl: got {:.3}, expected {:.3}", state.bml, expected_bmt);
        
        // VCB = T/2 = 2.5 m
        let expected_vcb = 2.5;
        assert!((state.vcb() - expected_vcb).abs() < 0.1,
                "VCB: got {:.3}, expected {:.3}", state.vcb(), expected_vcb);
        ...
        assert!((state.gmt.unwrap() - expected_gmt).abs() < 0.1,
                "GMT: got {:.3}, expected {:.3}", state.gmt.unwrap(), expected_gmt);
    }
    ...
    fn test_catamaran_two_disjoint_boxes() {
        ...
        assert!((state.waterplane_area - 100.0).abs() < 2.0,
                "Catamaran waterplane area: got {:.2}, expected 100.0", state.waterplane_area);
        
        // Expected TCF = 5.0 (midpoint between two hulls)
        // Hull1 centroid: (5.0, 0.0)
        // Hull2 centroid: (5.0, 10.0)
        // Combined: (5.0, 5.0)
        assert!((state.cob[1] - 5.0).abs() < 0.5,
                "Catamaran TCB: got {:.2}, expected 5.0", state.cob[1]);
    }
    ...
    fn test_two_joined_boxes() {
        ...
        assert!((state.waterplane_area - 100.0).abs() < 2.0,
                "Joined boxes waterplane area: got {:.2}, expected 100.0", state.waterplane_area);
        
        // Expected LCF = 5.0 (midpoint)
        assert!((state.cob[0] - 5.0).abs() < 0.3,
                "Joined boxes LCB: got {:.2}, expected 5.0", state.cob[0]);
    }
}
